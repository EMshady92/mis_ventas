...
function tipo_notificacion(id_tipo, id) {
    x = document.getElementById(id_tipo);
    aux = $('select[id=' + id_tipo + '] option:selected').text();
    input_email = document.getElementById('correo_' + id);
    input_direccion = document.getElementById('direccion_' + id);
    ...
    if (aux == "ESTRADOS") {
        ...
        var newText = "<div class='texts' style='clear:both; text-align:justify'><span style='font-family:Arial,Helvetica,sans-serif'><span style='font-size:16px'>Con efectos de <strong>emplazamiento </strong>le hago saber que dentro del expediente del Juicio Administrativo con n&uacute;mero <strong></strong>, se dict&oacute; el auto que enseguida se transcribe:</span></span></div>";
        CKEDITOR.instances["not_oficio"].setData(newText);
        $('#modalOficio').modal('show'); // abrir
        // input_email.required = false;
        //   input_direccion.required = true;
        // sintesis.required = false;
    } else if (aux = "BOLETÍN ELECTRONICO") {
        ...
        success: function (data) {
            if (data.acuerdos.length > 0) {
                data.acuerdos.forEach(acuerdo => {
                    ...
                    if (acuerdo.nombre != null && acuerdo.apellido_paterno != null && acuerdo.apellido_materno != null) {
                ...
                if (data.sentencias.length > 0) {
                    data.sentencias.forEach(sentencia => {
                        ...
                        cell4.innerHTML = '<td>' + sentencia.nombre + " " + sentencia.apellido_paterno + " " + sentencia.apellido_materno + " " + sentencia.razon_social + '</td>';
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.notificacion) {
                ...
                cell2.innerHTML = '<td><a href="' + data.notificacion.ruta + '" target="_blank" class="btn btn-danger waves-effect waves-light"   role="button" ">  <i class="ion ion-md-send"> ' + data.notificacion.tabla + '</i></a></td>';


                var row = tabla.insertRow(1);
                var cell1 = row.insertCell(0);
                var cell2 = row.insertCell(1);
                cell1.innerHTML = '<td>Usuario que envía la notificación:<td/>';
                cell2.innerHTML = '<td>' + data.notificacion.funcion2_funcion + ": " + data.notificacion.name_envia + " " + data.notificacion.user2_apellido_p + " " + data.notificacion.user2_apellido_m + '</td>';

                var row = tabla.insertRow(1);
                var cell1 = row.insertCell(0);
                var cell2 = row.insertCell(1);
                cell1.innerHTML = '<td>Usuario que recibe la notificación:<td/>';
                cell2.innerHTML = '<td>' + data.notificacion.funcion + ": " + data.notificacion.name + " " + data.notificacion.apellido_p + " " + data.notificacion.apellido_m + '</td>';
                var row = tabla.insertRow(1);
                var cell1 = row.insertCell(0);
                var cell2 = row.insertCell(1);
                cell1.innerHTML = '<td>Tipo de Notificación:<td/>';
                cell2.innerHTML = '<td class="notificacion">' + data.notificacion.tipo + '</td>';
                ...
                var cell1 = row.insertCell(0);
                var cell2 = row.insertCell(1);
                cell1.innerHTML = '<td>Fecha de la notificación:<td/>';
                cell2.innerHTML = '<td>' + data.notificacion.created_at + '</td>';
...
function subir_archivos(value) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        var x = $('#select' + i);
        documentos = ['Seleccione una opción...', 'ACUERDO', 'PROMOCION','AMPARO', 'SENTENCIA','RECURSO','PROMOCION DE RECURSO','AMPARO DE RECURSO','ACUERDO DE RECURSO','SENTENCIA DE RECURSO', 'DOCUMENTO'];
        ...
        success: function (data) {
            ...
            data.promoventes.forEach(promovente => {
                ...
                if (promovente.tipo == "AUTORIDAD") {
                    ...
                    option.text = promovente.nombre + " " + promovente.apellido_paterno;
                    option.value = promovente.id;
                }
    ...
    if (value != "") {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                Swal.fire({
                    /*  title: '¿La direccion del Actor seleccionado es correcta?'+ data.direcciones.calle, */
                    title: '¿Esta seguro de agregar al ' + tipo + ' seleccionado en la demanda?',
                    text: '',
                    icon: 'question',
                    showDenyButton: true,
                    showCancelButton: true,
...
function modal_direcciones(form, ruta, id, id_expediente) {
    dataString = $('#' + form).serialize(); // carga todos 
    $.ajax({
        type: "get",
        method: 'get',
...
function radio_est() {
    ...
    if (chk == true) {
        ...
        setTimeout(function () {
            ...
            if (tipo_user == "TRIBUNAL" && valida == 1) {
                cabezeras_graficos(0, 0, 'expedientes_etapa', 'exp_por_etapa');
                grafica_dona(0, 0, 'expedientes_etapa', 'exp_por_etapa');
                grafica_barras(0, 0, 'expedientes_por_usuarios', 'expedientes_persona');
                grafica_lineal(0, 0, 'recepcion_expedientes_char', 'recepcion_expedientes');
                grafica_dona(0, 0, 'tipo_recepcion_char', 'tipo_recepcion');
                //tablas_estadisticas(0, 0, 'autoridades_mas_demandas', 'datatable');
                aut_mas_demandas(0, 0, 'autoridades_mas_demandas_admin', 'padre2');
                grafica_area(0, 0, 'acuerdos_elaborados_char', 'acuerdos');
                grafica_barras(0, 0, 'usuarios_acuerdos', 'acuerdos_usuario');
                grafica_dona(0, 0, 'tipos_acuerdos', 'tipos_Acuerdos');
                grafica_area(0, 0, 'recepcion_promociones_char', 'promociones');
                grafica_dona(0, 0, 'tipos_promociones_char', 'tipos_promociones');
                grafica_area(0, 0, 'setencias_elaboradas_char', 'sentencias');
                grafica_dona(0, 0, 'tipos_sentencia_char', 'tipos_senetencias');
                actos_faltas_char(0, 0, 'actos_faltas_char', 'padre');
            } else if (tipo_user == "TRIBUNAL" && valida == 0) {
                cabezeras_graficos(0, 0, 'expedientes_etapa_admin', 'exp_por_etapa');
                grafica_dona(0, 0, 'expedientes_etapa_admin', 'exp_por_etapa');
                grafica_barras(0, 0, 'expedientes_por_usuarios_admin', 'expedientes_persona');
                grafica_lineal(0, 0, 'recepcion_expedientes_char_admin', 'recepcion_expedientes');
                grafica_dona(0, 0, 'tipo_recepcion_char_admin', 'tipo_recepcion');
                // tablas_estadisticas(0, 0, 'autoridades_mas_demandas_admin', 'datatable');
                aut_mas_demandas(0, 0, 'autoridades_mas_demandas_admin', 'padre2');
                grafica_area(0, 0, 'acuerdos_elaborados_char_admin', 'acuerdos');
                grafica_barras(0, 0, 'usuarios_acuerdos_admin', 'acuerdos_usuario');
                grafica_dona(0, 0, 'tipos_acuerdos_admin', 'tipos_Acuerdos');
                grafica_area(0, 0, 'recepcion_promociones_char_admin', 'promociones');
                grafica_dona(0, 0, 'tipos_promociones_char_admin', 'tipos_promociones');
                grafica_area(0, 0, 'setencias_elaboradas_char_admin', 'sentencias');
                grafica_dona(0, 0, 'tipos_sentencia_char_admin', 'tipos_senetencias');
                actos_faltas_char(0, 0, 'actos_faltas_char_admin', 'padre');
...
function cambiar_fecha_loby() {
    ...
    if (fecha_inicio <= fecha_fin && fecha_inicio != null && fecha_fin != null) {
        ...
        setTimeout(function () {
            ...
            if (tipo_user == "TRIBUNAL" && valida == 1) {
                cabezeras_graficos(fecha_inicio, fecha_fin, 'expedientes_etapa', 'exp_por_etapa');
                grafica_dona(fecha_inicio, fecha_fin, 'expedientes_etapa', 'exp_por_etapa');
                grafica_barras(fecha_inicio, fecha_fin, 'expedientes_por_usuarios', 'expedientes_persona');
                grafica_lineal(fecha_inicio, fecha_fin, 'recepcion_expedientes_char', 'recepcion_expedientes');
                grafica_dona(fecha_inicio, fecha_fin, 'tipo_recepcion_char', 'tipo_recepcion');
                //tablas_estadisticas(fecha_inicio, fecha_fin, 'autoridades_mas_demandas', 'datatable');
                aut_mas_demandas(0, 0, 'autoridades_mas_demandas_admin', 'padre2');
                grafica_area(fecha_inicio, fecha_fin, 'acuerdos_elaborados_char', 'acuerdos');
                grafica_barras(fecha_inicio, fecha_fin, 'usuarios_acuerdos', 'acuerdos_usuario');
                grafica_dona(fecha_inicio, fecha_fin, 'tipos_acuerdos', 'tipos_Acuerdos');
                grafica_area(fecha_inicio, fecha_fin, 'recepcion_promociones_char', 'promociones');
                grafica_dona(fecha_inicio, fecha_fin, 'tipos_promociones_char', 'tipos_promociones');
                grafica_area(fecha_inicio, fecha_fin, 'setencias_elaboradas_char', 'sentencias');
                grafica_dona(fecha_inicio, fecha_fin, 'tipos_sentencia_char', 'tipos_senetencias');
                actos_faltas_char(fecha_inicio, fecha_fin, 'actos_faltas_char', 'padre');
            } else if (tipo_user == "TRIBUNAL" && valida == 0) {

                cabezeras_graficos(fecha_inicio, fecha_fin, 'expedientes_etapa_admin', 'exp_por_etapa');
                grafica_dona(fecha_inicio, fecha_fin, 'expedientes_etapa_admin', 'exp_por_etapa');
                grafica_barras(fecha_inicio, fecha_fin, 'expedientes_por_usuarios_admin', 'expedientes_persona');
                grafica_lineal(fecha_inicio, fecha_fin, 'recepcion_expedientes_char_admin', 'recepcion_expedientes');
                grafica_dona(fecha_inicio, fecha_fin, 'tipo_recepcion_char_admin', 'tipo_recepcion');
                //tablas_estadisticas(fecha_inicio, 0, 'autoridades_mas_demandas_admin', 'datatable');
                aut_mas_demandas(fecha_inicio, fecha_fin, 'autoridades_mas_demandas_admin', 'padre2');
                grafica_area(fecha_inicio, fecha_fin, 'acuerdos_elaborados_char_admin', 'acuerdos');
                grafica_barras(fecha_inicio, fecha_fin, 'usuarios_acuerdos_admin', 'acuerdos_usuario');
                grafica_dona(fecha_inicio, fecha_fin, 'tipos_acuerdos_admin', 'tipos_Acuerdos');
                grafica_area(fecha_inicio, fecha_fin, 'recepcion_promociones_char_admin', 'promociones');
                grafica_dona(fecha_inicio, fecha_fin, 'tipos_promociones_char_admin', 'tipos_promociones');
                grafica_area(fecha_inicio, fecha_fin, 'setencias_elaboradas_char_admin', 'sentencias');
                grafica_dona(fecha_inicio, fecha_fin, 'tipos_sentencia_char_admin', 'tipos_senetencias');
                actos_faltas_char(fecha_inicio, fecha_fin, 'actos_faltas_char_admin', 'padre');
            }
        }, 1000);
...
function genera_pass(id) {
    $.ajax({
        type: "get",
        method: 'get',
        url: "/generarPasstxt/" + id,
        success: function (date) {
            var element = document.createElement('a');
            element.setAttribute('href', '/ssl/txt/' + date.firma.email + '.txt');
            element.setAttribute('download', date.firma.email + '.txt');
...
function guardar_noti_oficio() {
    ...
    document.getElementById('display_oficio').style.display = 'block';
    var tabla = document.getElementById('oficios');
    if (tabla.rows.length < 2) {
        var row = tabla.insertRow(1);
        row.style.backgroundColor = "white";
        var cell1 = row.insertCell(0);
        cell1.innerHTML = '<td><a type="button" class="btn btn-primary waves-effect waves-light btn-primary btn-sm" data-target="#modalOficio" data-dismiss="modal" data-toggle="modal" role="button"> <i class="mdi mdi-account-edit-outline"></i></a></td>';
    }
...
function eliminar_registro() {
    ...
    if (tipo != "" && id != "") {
        Swal.fire({
            title: '¿Esta seguro de eliminar este registro?, ya no se podra recuperar',
            icon: 'warning',
            showDenyButton: true,
            showCancelButton: false,
            confirmButtonColor: '#3085d6',
...
function guardar_persona_not(id) {
    ...
    $.ajax({
        ...
        success: function (date) {
            ...
            cell8.innerHTML = '<th> <span class="badge badge-danger">No se encontraron correos registrados</span> <select class="form-control"   name="correo_' + date.persona.id + '" id="correo_' + date.persona.id + '" style="width: 100%" multiple="multiple" > </select></th>';
            cell9.innerHTML = '<td><button type="button" class="btn btn-success waves-effect waves-light btn-success btn-sm" onclick="modal_aux_email(' + date.persona.id + ',' + id + ');""> <i class="mdi mdi-plus-box"></i></button> </td>';
            cell10.innerHTML = '<td> <span class="badge badge-danger">No se encontraron direcciones registradas</span> <select class="form-control" name="direccion_' + date.persona.id + '" id="direccion_' + date.persona.id + '" style="width: 100%" multiple="multiple" data-toggle="select2"></select></td>';
            cell11.innerHTML = '<td>  <button type="button" class="btn btn-success waves-effect waves-light btn-success btn-sm" onclick="modal_aux(' + date.persona.id + ',' + id + ');""> <i class="mdi mdi-plus-box"></i></button>      </td>';
